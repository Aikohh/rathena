...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, uint32 account_id) {
...
#ifdef VIP_ENABLE
		"SELECT `account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`pincode`, `pincode_change`, `passwd_type`, `vip_time`, `old_group` FROM `%s` WHERE `account_id` = %d",
#else
		"SELECT `account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`pincode`, `pincode_change`, `passwd_type` FROM `%s` WHERE `account_id` = %d",
#endif
	...
	Sql_GetData(sql_handle, 16, &data, nullptr); acc->passwd_type = (uint8)atoi(data);
#ifdef VIP_ENABLE
	Sql_GetData(sql_handle, 17, &data, nullptr); acc->vip_time = atol(data);
	Sql_GetData(sql_handle, 18, &data, nullptr); acc->old_group = atoi(data);
#endif
	Sql_FreeResult(sql_handle);
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new, bool refresh_token) {
	...
	{// insert into account table
...
#ifdef VIP_ENABLE
			"INSERT INTO `%s` (`account_id`, `userid`, `user_pass`, `sex`, `email`, `group_id`, `state`, `unban_time`, `expiration_time`, `logincount`, `lastlogin`, `last_ip`, `birthdate`, `character_slots`, `pincode`, `pincode_change`, `passwd_type`, `vip_time`, `old_group` ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
#else
			"INSERT INTO `%s` (`account_id`, `userid`, `user_pass`, `sex`, `email`, `group_id`, `state`, `unban_time`, `expiration_time`, `logincount`, `lastlogin`, `last_ip`, `birthdate`, `character_slots`, `pincode`, `pincode_change`, `passwd_type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
#endif
			db->account_db)
		...
		||  SQL_SUCCESS != stmt.BindParam(16, SQLDT_UCHAR,     (void*)&acc->passwd_type,     sizeof(acc->passwd_type))
#ifdef VIP_ENABLE
		||  SQL_SUCCESS != stmt.BindParam(17, SQLDT_LONG,       (void*)&acc->vip_time,         sizeof(acc->vip_time))
		||  SQL_SUCCESS != stmt.BindParam(18, SQLDT_INT32,        (void*)&acc->old_group,        sizeof(acc->old_group))
#endif
	...
	{// update account table
		if( SQL_SUCCESS != stmt.Prepare( 
#ifdef VIP_ENABLE
			"UPDATE `%s` SET `userid`=?,`user_pass`=?,`sex`=?,`email`=?,`group_id`=?,`state`=?,`unban_time`=?,`expiration_time`=?,`logincount`=?,`lastlogin`=?,`last_ip`=?,`birthdate`=?,`character_slots`=?,`pincode`=?, `pincode_change`=?, `passwd_type`=?, `vip_time`=?, `old_group`=? WHERE `account_id` = '%d'",
#else
			"UPDATE `%s` SET `userid`=?,`user_pass`=?,`sex`=?,`email`=?,`group_id`=?,`state`=?,`unban_time`=?,`expiration_time`=?,`logincount`=?,`lastlogin`=?,`last_ip`=?,`birthdate`=?,`character_slots`=?,`pincode`=?, `pincode_change`=?, `passwd_type`=? WHERE `account_id` = '%d'",
#endif
		...
		||  SQL_SUCCESS != stmt.BindParam(15, SQLDT_UCHAR,     (void*)&acc->passwd_type,     sizeof(acc->passwd_type))
#ifdef VIP_ENABLE
